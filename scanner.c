...
static TokenType identifierType() {
  ...
  for (int i=0; keywords[i].ident != NULL; i++) {
    if (length == keywords[i].length && 
        memcmp(scanner.start, keywords[i].ident, length) == 0) 
    {
...
static Token string() {
  while (peek() != '"' && !isAtEnd()) {
    if (peek() == '\\') advance(); // skip escape charcters
    if (peek() == '\n') scanner.line++;
    advance();
  }
  ...
  switch (c) {
    ...
    case '<':
      switch(peek()) {
        ...
        case '<': advance(); return makeToken(TOKEN_LESS_LESS);
        default: return makeToken(TOKEN_LESS);
      }
    case '>':
      switch(peek()) {
        case '=': advance(); return makeToken(TOKEN_GREATER_EQUAL);
        case '>': advance(); return makeToken(TOKEN_GREATER_GREATER);
